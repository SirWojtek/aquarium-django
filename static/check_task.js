...
$( document ).ready(function() {
    // copied from external HTML

    for (i = 0; i < schedule_list.length; i++) {
        schedule.push(get_dates(schedule_list[i]));
    }

    if (currently_edited_task != null)
        edited_task = get_dates(currently_edited_task);
});
...
function get_dates(task) {
    ...
    return { id : task.id, start_date : start_date, end_date: end_date };
}
...
function is_task_within_another_in_schedule(task) {
    for (i = 0; i < schedule.length; i++) {
        if (is_currently_edited_task(edited_task, schedule[i])) {
            continue;
        }
